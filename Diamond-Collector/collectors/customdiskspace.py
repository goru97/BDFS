...
class CustomDiskSpaceCollector(diamond.collector.Collector):

    def get_default_config_help(self):
        config_help = super(CustomDiskSpaceCollector, self).get_default_config_help()
        config_help.update({
    ...
    def get_default_config(self):
        ...
        """
        config = super(CustomDiskSpaceCollector, self).get_default_config()
        config.update({
            # Enabled by default
            'enabled': 'True',
            'path': 'diskspace',
    ...
    def __init__(self, config, handlers):
        super(CustomDiskSpaceCollector, self).__init__(config, handlers)
    ...
    def collect(self):
        ...
        for key, info in results.iteritems():
            ...
            for unit in self.config['byte_unit']:
                ...
                metric_value = (float(blocks_avail) / float(blocks_total)) * 100
